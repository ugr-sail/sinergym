...
@pytest.fixture
def example_value():
	return 40        #Normally we will use for complex items like instances, DB rows, etc.
...
def test_divisible10(example_value):
	assert ((example_value%10) == 0)  # If we change the fixture state only is applied in that test function
