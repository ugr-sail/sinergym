...
def test_reset(simulator):
	...
	assert simulator._env_working_dir_parent.split("/")[-1] == "Eplus-env-"+simulator._env_name+"-res1"
	assert simulator._epi_num==0
	assert simulator._episode_existed
	path_list=simulator._eplus_working_dir.split("/")
	assert path_list[-2]+"/"+path_list[-1] == "Eplus-env-"+simulator._env_name+"-res1/Eplus-env-sub_run"+str(simulator._epi_num+1)
...
def test_get_eplus_working_folder(simulator):
	...
	expected="Eplus-env-TEST-res1/Eplus-env-sub_run"+str(simulator._epi_num+2) 
...
def test_get_file_name(simulator):
	file_path="./example/dict1/dict2/file.txt"
	assert simulator._get_file_name(file_path) == "file.txt"
...
def test_assembleMsg(simulator):
	...
	msg = simulator._assembleMsg(header, 0, len(action), 0, 0, curSimTim, Dblist)
...
def test_disassembleMsg(simulator):
	...
	(version, flag, nDb, nIn, nBl, curSimTim, Dblist) = simulator._disassembleMsg(msg)
	assert version==0
	assert flag==0
	assert nDb==2
	assert nIn==0
