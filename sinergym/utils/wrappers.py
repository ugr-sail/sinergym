...
class RadiantLoggerWrapper(BaseLoggerWrapper):
    ...
    def __init__(self, env: Env, storage_class: Callable = LoggerStorage):
        ...
        self.summary_metrics = [
            ...
            'cumulative_power_demand',
            'comfort_violation_time(%)',
            'mean_compressor_cycling_per_day',
            'std_compressor_cycling_per_day',
            'mean_compressor_starts_per_day',
            'std_compressor_starts_per_day',
            'mean_crf_state_duration_per_day',
            'std_crf_state_duration_per_day',
            'length(timesteps)',
            'time_elapsed(hours)',
            'terminated',
            'truncated',
...
class ExtremeFlowControlWrapper(gym.ActionWrapper):
    ...
    def action(self, action):
        action_ = np.array(deepcopy(action))
        if len(action_) > 1:  # excluding case 1
